...
/* game start and death variables */
var title = true;
var rightLegR;
var leftLegR;
...
function preload() {
  space = loadImage('1space.jpg');
  mount = loadImage('1mountain.jpg')
}
...
function scrollScene() {
    ...
    if (scene4y <= 0) {
        if(scroll()) {
                scene1y -= player.getVelocity()*(.1);
                scene2y -= player.getVelocity()*(.1);
                scene3y -= player.getVelocity()*(.1);
                scene4y -= player.getVelocity()*(.1);
...
function scene2()
{    
    image(mount, -1, scene2y+1, 501, 601);
